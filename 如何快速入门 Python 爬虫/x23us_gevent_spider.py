...
class X23usSpider:
    ...
    def download_and_parse_list_page(self, page):
        ...
        for tr in trs[1:]:
            ...
            # 进一步请求这本小说的主页，获取章节列表
            tasks.append((novel_url, novel_id))
            # self.download_and_parse_novel_page(novel_url, novel_id)
            # break
        # 创建协程池，并发下载小说
        pool = Pool(5)
        for novel_url, novel_id in tasks:
            pool.spawn(self.download_and_parse_novel_page, novel_url, novel_id)
        pool.join()
